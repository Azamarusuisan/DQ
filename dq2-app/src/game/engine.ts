...
battlemaster:{id:7,name:'バトルマスター',statMod:{str:1.4,agi:1.1,maxHp:1.2,maxMp:0.4},skills:[[2,'hayabusagiri'],[4,'midareuchi'],[6,'kyodaitsumuji'],[8,'gigaslash']],req:{warrior:5,fighter:5}},
sage:{id:8,name:'賢者',statMod:{str:0.8,agi:1.0,maxHp:0.9,maxMp:1.5},skills:[[2,'begiragon'],[4,'mahyado2'],[6,'behomaraa'],[8,'madante']],req:{mage:5,priest:5}},
magicknight:{id:9,name:'魔法戦士',statMod:{str:1.1,agi:1.0,maxHp:1.0,maxMp:1.1},skills:[[2,'flameslash'],[4,'iceslash'],[6,'mppassion'],[8,'baikicrush']],req:{warrior:5,mage:5}},
paladin:{id:10,name:'パラディン',statMod:{str:1.0,agi:0.9,maxHp:1.3,maxMp:1.1},skills:[[2,'palashield'],[4,'grandcross_s'],[6,'megante'],[8,'palaguard']],req:{priest:5,fighter:5}},
ranger:{id:11,name:'レンジャー',statMod:{str:1.0,agi:1.2,maxHp:1.0,maxMp:0.9},skills:[[2,'fieldguard'],[4,'toxicbreath'],[6,'mofusen'],[8,'deathcutter']],req:{thief:5,fighter:5}},
superstar:{id:12,name:'スーパースター',statMod:{str:0.9,agi:1.1,maxHp:0.9,maxMp:1.2},skills:[[2,'starlight'],[4,'grandbocce'],[6,'hussledance2'],[8,'moonlight']],req:{dancer:5,mage:5}},
hero_job:{id:13,name:'勇者',statMod:{str:1.3,agi:1.2,maxHp:1.3,maxMp:1.3},skills:[[2,'gigaslash2'],[4,'jigopark'],[6,'grandcross'],[8,'madante2']],req:{battlemaster:5,sage:5}}};
...
return Array.from({length:99},(_,i)=>{const lv=i+1;const exp=Math.floor(lv*lv*lv*0.5+lv*lv*25+lv*80);const sp=spells.filter(([l])=>l===lv).map(([,s])=>s);return[exp,hp0+Math.floor(hpG*i),mp0+Math.floor(mpG*i),str0+Math.floor(strG*i),agi0+Math.floor(agiG*i),sp]})}
...
const MONSTERS=[
// Zone 0: ローレシア周辺
{name:'スライム',hp:8,mp:0,atk:6,def:3,agi:3,exp:4,gold:4,spells:[],zone:[0],drop:{type:'item',id:0,rate:0.25}},
{name:'おおなめくじ',hp:14,mp:0,atk:10,def:4,agi:2,exp:6,gold:6,spells:[],zone:[0],drop:{type:'item',id:1,rate:0.125}},
{name:'アイアンアント',hp:16,mp:0,atk:12,def:6,agi:4,exp:8,gold:8,spells:[],zone:[0],drop:{type:'item',id:0,rate:0.125}},
// Zone 1: サマルトリア～ムーンペタ
{name:'ドラキー',hp:12,mp:0,atk:9,def:5,agi:6,exp:5,gold:5,spells:[],zone:[0,1],drop:{type:'item',id:2,rate:0.0625}},
{name:'バブルスライム',hp:18,mp:0,atk:14,def:6,agi:4,exp:10,gold:10,spells:[],zone:[1],drop:{type:'item',id:1,rate:0.125}},
{name:'マンドリル',hp:28,mp:0,atk:18,def:10,agi:8,exp:15,gold:18,spells:[],zone:[1],drop:{type:'weapon',id:9,rate:0.03}},
{name:'リビングデッド',hp:25,mp:0,atk:19,def:10,agi:5,exp:15,gold:20,spells:[],zone:[1],drop:{type:'armor',id:1,rate:0.0625}},
{name:'まじゅつし',hp:25,mp:10,atk:16,def:10,agi:14,exp:18,gold:24,spells:['gira'],zone:[1],drop:{type:'item',id:2,rate:0.125}},
// Zone 2: ロンダルキアへの洞窟
{name:'バピラス',hp:38,mp:8,atk:28,def:17,agi:14,exp:22,gold:28,spells:['rariho'],zone:[2],drop:{type:'weapon',id:15,rate:0.03}},
{name:'ドラゴン',hp:55,mp:0,atk:33,def:26,agi:14,exp:35,gold:48,spells:['gira'],zone:[2],drop:{type:'shield',id:5,rate:0.03}},
{name:'キラーマシン',hp:90,mp:0,atk:44,def:52,agi:25,exp:70,gold:60,spells:[],zone:[2,3],drop:{type:'weapon',id:10,rate:0.03}},
{name:'フレイム',hp:45,mp:8,atk:33,def:22,agi:20,exp:30,gold:38,spells:['gira'],zone:[2],drop:{type:'armor',id:6,rate:0.03}},
...
// ボス
{name:'ハーゴン',hp:500,mp:80,atk:85,def:65,agi:40,exp:3000,gold:500,spells:['begirama','behoimi','ionazun'],zone:[],boss:true},
{name:'シドー',hp:600,mp:99,atk:90,def:70,agi:45,exp:5000,gold:0,spells:['ionazun','behoimi'],zone:[],boss:true},
// Zone 4: 隠しダンジョン（裏ダン）
...
if(G.wagon)G.wagon.forEach(c=>{if(!c.alive)return;c.exp+=Math.floor(te/2);if(c.lvUp())ms.push(c.name+'(馬車) Lv'+c.level+'に UP！')});bs.enemies.forEach(e=>{if(e.name==='ハーゴン'){G.flags.hargonDown=true;G.party.forEach(c=>{if(c.alive){c.hp=c.maxHp;c.mp=c.maxMp;c.status=[]}});ms.push('ハーゴンの呪いが解けた！\n力が湧き上がる！\nHP MPが かいふくした！')}if(e.name==='シドー'){G.flags.sidoDown=true;ms.push('へいわが おとずれた…！')}if(e.name==='ゾーマ'){G.flags.zomaDown=true;ms.push('ゾーマは 闇に還っていった…！')}if(e.name==='エスターク'){G.flags.estarkDown=true;ms.push('エスタークは 再び眠りについた…！')}if(e.name==='ダークドレアム'){G.flags.dreaamDown=true;ms.push('ダークドレアム「見事だ…\nお前たちこそ 真の勇者だ。」')}
...
function newGame(heroName){G.hero=new Chara(0,heroName||'ローレシアのおうじ',HERO_LV);G.hero.weapon=1;G.hero.armor=1;G.party=[G.hero];G.gold=150;G.items=[{id:0,count:3}];G.mapId='world';G.px=12;G.py=15;G.pdir=0;G.flags={};G.visited=['loracia'];G.prince=null;G.princess=null;G.hasShip=false;G.onShip=false;G.chests={};G.zukan={monsters:{},items:{}};G.wagon=[];initMaps();initNPCs();G.state=GS.DIALOG;showMsg('ローレシア王「おお ローレシアのおうじよ！\n邪教の教祖ハーゴンが\n世界を闇に染めようとしている。」',()=>{showMsg('ローレシア王「ロトの血を引く3人の\n勇者が揃えば 奴を倒せるはず。\nまずは南のサマルトリア城で\n王子を仲間にするのじゃ！」',()=>{showMsg('ローレシア王「そうじゃ…\n北にダーマ神殿がある。\n転職の力で己を鍛えれば\nさらに強くなれるじゃろう。」',()=>{G.state=GS.EXPLORE})})})}
...
// 詰み防止: 必須アイテム自動付与
if(!G.flags.princessJoined&&!G.items.some(it=>it.id===8))G.items.push({id:8,count:1});
if(!G.hasShip&&!G.items.some(it=>it.id===7))G.items.push({id:7,count:1});
